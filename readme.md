# Jruing 技术学习笔记

这是一个个人技术学习和实践记录的知识库，涵盖了前端开发、后端开发、运维、AI、数据库和Linux等多个技术领域。所有内容均为中文记录，格式统一，便于查阅和学习。

## 📁 项目结构

```
Jruing/
├── readme.md (本文件)
└── [6个主要分类目录]
    ├── 开发/
    │   ├── 前端/
    │   │   ├── 基础三剑客/
    │   │   │   └── javascript/
    │   │   │       ├── 元素选择器.md
    │   │   │       └── forEach数组循环.md
    │   │   ├── React/ (空)
    │   │   └── Vue/ (空)
    │   ├── Golang/
    │   │   └── go-zero/
    │   │       ├── 安装.md
    │   │       └── api语法.md
    │   ├── Java/ (空)
    │   └── Python/
    │       ├── 基础/
    │       │   ├── 继承与多继承MRO.md
    │       │   ├── pathlib用法.md
    │       │   ├── pydantic用法.md
    │       │   ├── python日志.md
    │       │   ├── struct用法.md
    │       │   ├── uv安装Python.md
    │       │   └── zipapp用法.md
    │       ├── 设计模式/
    │       │   ├── 单例模式.md
    │       │   └── 工厂模式.md
    │       ├── 数据分析/
    │       │   └── Pandas用法.md
    │       ├── 自动化/
    │       │   └── playwright/
    │       │       ├── 目录.md
    │       │       ├── playwright安装.md
    │       │       ├── playwright防自动化检测.md
    │       │       ├── playwright基础.md
    │       │       └── playwright进阶.md
    │       └── Web/
    │           ├── Django/ (空)
    │           ├── FastAPI/
    │           │   └── 安装.md
    │           └── Flask/ (空)
    ├── 日常工具/
    │   └── RustDesk 私有化部署.md
    ├── 数据库/
    │   ├── MySQL/ (空)
    │   ├── PostgreSQL/ (空)
    │   └── Redis/
    │       └── 安装及基础.md
    ├── 运维/
    │   ├── 监控/
    │   │   ├── 日志监控/
    │   │   │   └── 基于Grafana+Loki+Promatil搭建日志分析平台.md
    │   │   ├── Prometheus/
    │   │   │   ├── Prometheus笔记-安装.md
    │   │   │   ├── Prometheus笔记-安装blackbox_exporter.md
    │   │   │   ├── Prometheus笔记-安装Node_exporter.md
    │   │   │   ├── Prometheus笔记-告警规则配置.md
    │   │   │   ├── Prometheus笔记-监控docker容器.md
    │   │   │   ├── Prometheus笔记-配置文件字段解析.md
    │   │   │   ├── Prometheus笔记-设置Basic_auth登录校验.md
    │   │   │   ├── Prometheus笔记-file_sd_config.md
    │   │   │   ├── Prometheus笔记-Grafana可视化.md
    │   │   │   └── Prometheus笔记-Label标签.md
    │   │   └── Uptime-kuma 监控.md
    │   ├── 进程管理/
    │   │   └── Supervisor 学习笔记.md
    │   ├── 容器化/
    │   │   ├── Docker/
    │   │   │   └── 环境搭建与安装.md
    │   │   └── Kubernetes/
    │   │       ├── 目录.md
    │   │       ├── kubernetes学习笔记-安装.md
    │   │       ├── kubernetes学习笔记-ConfigMap.md
    │   │       ├── kubernetes学习笔记-Deployment.md
    │   │       ├── kubernetes学习笔记-helm.md
    │   │       ├── kubernetes学习笔记-Ingress.md
    │   │       ├── kubernetes学习笔记-namespace.md
    │   │       ├── kubernetes学习笔记-Pod.md
    │   │       ├── kubernetes学习笔记-ReplicaSet.md
    │   │       └── kubernetes学习笔记-Service.md
    │   ├── Caddy/ (空)
    │   ├── Jenkins/
    │   │   ├── 部署Github项目.md
    │   │   ├── 环境搭建与安装.md
    │   │   ├── 目录.md
    │   │   ├── Freestyle Job.md
    │   │   ├── Jenkinsfile.md
    │   │   ├── Pipline.md
    │   │   └── Trigger.md
    │   ├── Minio/
    │   │   └── 环境搭建与安装.md
    │   ├── Nginx/
    │   │   └── 环境搭建与安装.md
    │   └── Traefik/
    │       ├── Traefik学习笔记-安装.md
    │       ├── Traefik学习笔记-概念详解（基于 Docker Labels 配置）.md
    │       ├── Traefik学习笔记-重定向配置.md
    │       └── Traefik学习笔记-自定义中间件.md
    ├── AI/
    │   ├── Claude Code+Claude Code Router.md
    │   ├── Gemini-cli.md
    │   ├── Coze/
    │   │   └── 安装 Coze Studio.md
    │   ├── Eino/ (空)
    │   ├── Google ADK/
    │   │   ├── Google ADK安装.md
    │   │   └── Google ADK实现简历打分助手.md
    │   └── 提示词/
    │       ├── 豆包-生成架构图.md
    │       ├── 豆包-丝控.md
    │       ├── 豆包-KFC图片.md
    │       ├── Banana提示词-文章变成卡通信息图.md
    │       └── gork-涩涩.md
    └── Linux/
        └── 基础/
            ├── linux学习笔记-搭建FTP服务.md
            ├── linux学习笔记-服务器基础信息查询脚本.md
            ├── linux学习笔记-更换镜像源.md
            ├── linux学习笔记-免密登录.md
            ├── linux学习笔记-终端美化-zsh.md
            ├── linux学习笔记-awk.md
            ├── linux学习笔记-cgroup(二).md
            ├── linux学习笔记-cgroup(一).md
            ├── linux学习笔记-grep.md
            ├── linux学习笔记-iptables.md
            ├── linux学习笔记-jq.md
            ├── linux学习笔记-nano用法.md
            ├── linux学习笔记-rsync数据同步.md
            ├── linux学习笔记-screen.md
            ├── linux学习笔记-sed手册.md
            ├── linux学习笔记-SSH隧道代理.md
            ├── linux学习笔记-sudo.md
            ├── linux学习笔记-Systemd.md
            ├── linux学习笔记-tmux.md
            ├── linux学习笔记-Ubuntu常用软件安装.md
            └── linux学习笔记-ufw.md
```

## 📊 项目统计

- **总文件数量**: 100+ 个 Markdown 文件
- **总目录数量**: 40+ 个子目录
- **主要分类**: 6个核心技术领域

## 🚀 快速导航

### 开发
- **前端**: JavaScript基础、React、Vue
- **后端**: Golang、Python、Java、C++
- **自动化**: Playwright测试框架

### 运维
- **监控**: Prometheus、Grafana、Uptime-kuma
- **容器化**: Docker、Kubernetes、Helm
- **CI/CD**: Jenkins、Traefik、Nginx
- **进程管理**: Supervisor

### AI
- **AI工具**: Claude、Gemini、Google ADK、Coze
- **提示词**: 多种AI提示词模板和技巧

### 数据库
- **MySQL、PostgreSQL、Redis**

### Linux
- **基础命令**: 20+个常用Linux命令和配置

## 📝 使用说明

1. **搜索**: 使用Obsidian的搜索功能快速查找相关内容
2. **标签**: 每个文件都包含相关标签，便于分类查找
3. **链接**: 文档之间有相互链接，便于知识体系构建

## 🔧 技术栈

- **笔记工具**: Obsidian
- **版本控制**: Git
- **内容格式**: Markdown

## 📄 许可证

本项目为个人学习笔记，仅供学习交流使用。

## 🤝 贡献

欢迎提出改进建议和补充内容，共同完善这个技术学习库。

---

*最后更新: 2026-01-27*